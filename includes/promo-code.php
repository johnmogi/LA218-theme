...
// Include installation and admin functionality
require_once get_stylesheet_directory() . '/includes/promo-code-install.php';
require_once get_stylesheet_directory() . '/includes/promo-code-admin.php';
...
function lilac_promo_code_display($atts = []) {
    ...
    // Default Hebrew text - Using HTML entities for better compatibility
    $defaults = [
    ...
    <div class="lilac-promo-code-container">
        <?php if (!empty($atts['title'])) : ?>
            <h2 class="lilac-promo-code-title" style="text-align: right; direction: rtl;"><?php echo html_entity_decode(esc_html($atts['title']), ENT_QUOTES, 'UTF-8'); ?></h2>
        <?php endif; ?>
        
        <?php if (!empty($atts['description'])) : ?>
            <p class="lilac-promo-code-description" style="text-align: right; direction: rtl;"><?php echo html_entity_decode(esc_html($atts['description']), ENT_QUOTES, 'UTF-8'); ?></p>
        <?php endif; ?>
        ...
        <form method="post" class="lilac-promo-code-form">
            ...
            <div class="lilac-form-group">
                <input type="text" 
                   ...
                   placeholder="<?php echo esc_attr(html_entity_decode($atts['input_placeholder'], ENT_QUOTES, 'UTF-8')); ?>" 
            ...
            <div class="lilac-form-group">
                <input type="submit" 
                       ...
                       value="<?php echo esc_attr(html_entity_decode($atts['button_text'], ENT_QUOTES, 'UTF-8')); ?>" 
...
/**
 * Validate a promotion code against the registration codes system
 * 
...
function lilac_validate_promo_code($code, $course_id = 898) {
    ...
    $user_id = get_current_user_id();
    $table_name = $wpdb->prefix . 'edc_school_promo_codes'; // Use existing table with edc prefix
    $usage_table = $wpdb->prefix . 'edc_school_promo_code_usage'; // Usage tracking table
    ...
    try {
        ...
        // First check the usage tracking table
        $user_used_code = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $usage_table WHERE code_id = %d AND user_id = %d",
            $code_data->id,
            $user_id
        ));
        
        if ($user_used_code && $user_used_code > 0) {
            $wpdb->query('ROLLBACK');
            return [
                'status' => false,
        ...
        // Also check if the code is one-time use globally and already used by someone
        if ($code_data->is_used == 1 && $code_data->max_uses <= 1) {
            $wpdb->query('ROLLBACK');
            return [
                'status' => false,
                'message' => 'הקוד כבר נוצל ואינו פעיל יותר.',
                'error_type' => 'code_used'
            ];
        }
        
        // Update the code usage count and mark as used if it reaches max uses
        $current_time = current_time('mysql');
        $new_used_count = $code_data->used_count + 1;
        
        // Prepare updates for the promo code table
        $update_data = [
            'used_count' => $code_data->used_count + 1,
            'updated_at' => current_time('mysql')
        ];
        
        // Mark as used if this is a one-time use code
        ...
        // If this will be the last usage, update status to inactive
        ...
        if ($updated === false) {
            $wpdb->query('ROLLBACK');
            error_log('Failed to update promo code: ' . $wpdb->last_error);
            return [
                'status' => false,
        ...
        // If insert fails but it's due to duplicate entry, we can continue (user already used the code)
        if ($inserted === false && strpos($wpdb->last_error, 'Duplicate entry') === false) {
            $wpdb->query('ROLLBACK');
...
function lilac_promo_code_redirect() {
    // Initialize debug logging
    $debug = defined('WP_DEBUG') && WP_DEBUG;
    $log_prefix = '[Promo Code Redirect] ';
    ...
    $user_id = get_current_user_id();
    $redirect_url = get_transient('lilac_promo_redirect_' . $user_id);
    
    if ($redirect_url) {
        // Delete the transient so it only redirects once
        delete_transient('lilac_promo_redirect_' . $user_id);
