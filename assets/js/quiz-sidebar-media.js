...
(function($) {
    ...
    function initQuizSidebarMedia() {
        ...
        if (!$('#question-media').length) {
            $('.ld-quiz-sidebar-content').html('<div id="question-media"><div class="media-placeholder"><div class="media-icon"><i class="fas fa-image"></i></div><div class="media-message">המדיה תופיע כשתפתח/י את המבחן</div></div></div>');
        }
    ...
    function loadQuestionMedia(questionId) {
        ...
        $('#question-media').html('<div class="media-content question-media-image"><img src="http://lilacquiz.local/wp-content/uploads/2025/05/noPic.png" alt="טוען..." class="fallback-image"></div>');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    // Show error message
                    $('#question-media').html('<div class="media-error">שגיאה בטעינת המדיה</div>');
                }
            },
